$(() => {
        ...
        fetch(url)
            ...
            .then(json => {
                ...
                switch (json.weather[0].main) {
                    case "Clear": icon.html(`<span class="fas fa-sun"></span><span ${style}">${description}</span>`); break;
                    case "Snow": icon.html(`<span class="fas fa-snowflake"></span><span ${style}">${description}</span>`); break;
                    case "Rain": icon.html(`<span class="fas fa-cloud-rain"></span><span ${style}">${description}</span>`); break;
                    case "Drizzle": icon.html(`<span class="fas fa-cloud-rain"></span><span ${style}">${description}</span>`); break;
                    case "Thunderstorm": icon.html(`<span class="fas fa-bolt"></span><span ${style}">${description}</span>`); break;
                    case "Clouds": icon.html(`<span class="fas fa-cloud"></span><span ${style}">${description}</span>`); break;
                    default: icon.html(`<span ${style}">${description}</span>`)
                }
            })
        // clear input
        sBox_input.value = '';
    ...
    sBox_input.addEventListener('keyup', e => (e.keyup === 13) ? getWeather() : '');
});